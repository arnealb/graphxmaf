...
ONEDRIVE_FILE_NAME = "supplier1_delivery_report_Q2_2026.txt"
ONEDRIVE_FILE_CONTENT = """supplier1 - Q2 Delivery Report
================================
...
def inject_onedrive(token):
    ...
    r = requests.put(
        f"https://graph.microsoft.com/v1.0/users/{MAILBOX}/drive/root:/{ONEDRIVE_FILE_NAME}:/content",
        headers=headers,
        data=ONEDRIVE_FILE_CONTENT.encode("utf-8"),
    )
    r.raise_for_status()
    print(f"[1/1] ✅ {ONEDRIVE_FILE_NAME} geüpload naar OneDrive root")
    print()
